...
namespace CFDIProcessor
{
    ...
    class Program
    {
        ...
        {
            Console.WriteLine("╔════════════════════════════════════════════════════════════╗");
            Console.WriteLine("║         Procesador de CFDI - Versión Unificada            ║");
            Console.WriteLine("╚════════════════════════════════════════════════════════════╝");
            Console.WriteLine();
            ...
            try
            {
                ...
                // Procesar archivos con el procesador unificado
                ProcessFiles(folderPath, configuration);
        ...
        /// <summary>
        /// Procesa los archivos XML de la carpeta especificada usando el procesador unificado
        /// </summary>
        private static void ProcessFiles(string folderPath, IConfiguration configuration)
        {
            using (var context = new DescargaCfdiGfpContext())
            {
                ...
                Console.WriteLine("📋 Procesando archivos CFDI...");
                Console.WriteLine();
                
                var unifiedProcessor = new UnifiedCfdiProcessor(context, configuration);
                unifiedProcessor.ProcessXmlFilesFromFolder(folderPath);
            }
        }
    }
}
